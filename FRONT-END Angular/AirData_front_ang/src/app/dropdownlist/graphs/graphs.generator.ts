...
export function generateChartOptions(seriesData: ApexAxisChartSeries = [], categoriesDates: string[] = [], type: 'airQ' | 'temp' | 'hum'): ChartOptions {

    //configurate local variables
    //global
    const color_opacity = .09;
    const title: string = (type === 'airQ' ? "Kvalita Vzuchu" : (type === 'temp' ? "Teplota" : "Vlhkost"));
